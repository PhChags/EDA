...
TFunc *busca_binaria(int chave, FILE *in, int inicio, int fim) {//o inicio no arquivo o primeiro registro não é zero e sim 1, diferente de vetores
	TFunc *f = NULL;//cria uma struct funcionario e inicializa com NULL
	int cod = -1;//inicializa o cod para poder entrar no while, utiliza o -1 pois sabe-se que não existem codigos negativos no arquivo
	while (inicio <= fim && cod != chave) {
		int meio = trunc((inicio + fim) / 2);//trunca para pegr o menor, caso a soma seja um numero par
		printf("Inicio: %d; Fim: %d; Meio: %d\n", inicio, fim, meio);
		fseek(in, (meio - 1) * tamanho_registro(), SEEK_SET);//posiciona o ponteiro no meio do arquivo
		f = le_funcionario(in);//le o funcionario do meio
		printf("cod: %d\nnome: %s\n salario: %lf\n", f->cod, f->nome, f->salario);
		if (f) {//se não for NULL
			cod = f->cod;//atualiza o cod
			if (cod > chave) {
				fim = meio - 1;//vai para a metade de cima, subindo o fim e encurtando o arquivo
				free(f);//free para desalocar os ponteiros que ficaram soltos
			} else if(cod < chave){
				inicio = meio + 1;//vai para metade de baixo, descendo o inicio e encuratndo o arquivo
				free(f);
			}
		}
	}
	...
	//Metodo sequencial, mas normalmente eh feito de maneira recurssivo
}
